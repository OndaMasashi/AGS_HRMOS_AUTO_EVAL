...
### 高: 自動NG登録の初回の定期実行を見届ける

2026-09-07 に `dry_run: false` へ切り替え、**定期実行（平日 12:30 / 17:30）から自動登録が
始まる状態**にした。初回の実行後、メール通知に出る内訳（登録／対象期間外／評価フォームなし／
判定不能／失敗）を必ず確認すること。

- 「失敗」が出ていたら `data/debug/` の `ng_*` を見てセレクタの破損を疑う
- 想定と違う件数が登録されていたら、`config.yaml` の `dry_run` を `true` に戻して止める
