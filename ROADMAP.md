...
### 高: 新しい○基準での初回の自動NG登録を見届ける

2026-09-09 に `first_pass_criteria` を引き上げた（○率 52%→35% 見込み）。
基準を上げると △ だった応募者が × に落ちて**自動NG登録の対象が増える**。
`dry_run: false`（本番登録する状態）のまま運用に入るため、**次回の定期実行の結果を必ず確認すること**。

- メール通知の内訳（登録／対象期間外／評価フォームなし／判定不能／失敗）が想定どおりか
- 想定より件数が多い場合は `config.yaml` の `dry_run` を `true` に戻せば登録だけ止まる
- **HRMOS への登録は取り消せない**ため、気づくのが遅れるほど影響が広がる

閾値の決定根拠と通過率の試算は [改修履歴](improvement_list/2026-09-09_fix_llm_model_drift.md)。
