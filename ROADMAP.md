...
### 中: 自動NG登録は dry-run のままで、定期実行での本番登録は始まっていない

2026-09-07 に稼働中の `config.yaml` へ `hrmos_evaluation` を追加し、`enabled: true` /
**`dry_run: true`** で動いている。この状態では対象者の判定とフォーム操作までは行うが
**登録はしない**（ログとメールに「dry-run確認 N件」と出るだけ）。

本番登録を始めるには `dry_run: false` に変える。まず dry-run で数回まわし、メール通知に出る内訳
（登録／対象期間外／評価フォームなし／判定不能）が想定どおりかを確認してから切り替えること。

なお溜まっていた候補者12名は 2026-09-07 に手動スクリプトで登録済み（詳細は改修履歴）。
