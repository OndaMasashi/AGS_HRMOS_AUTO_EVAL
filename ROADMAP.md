...
2026-09-06 に本番登録を1件試して成功した。ただし登録後の画面を調べたところ、
...
### 高: 年齢・性別の扱い（AI が評価を拒否した理由の1つ）

2026-09-16/17 に Claude が評価を拒否した理由のうち、ツールでは直していないもの。出力スキーマの
`applicant_gender` / `applicant_age`、年齢帯ごとの閾値、面接質問の「35歳以上であれば」分岐。
労働施策総合推進法9条（募集・採用時の年齢制限の原則禁止）と男女雇用機会均等法を名指ししていた。
**ツールの修正ではなく運用判断**（社労士確認を推奨）。住所マスクと評価AIの隔離は対応済み
（[2026-09-21 AI評価の拒否への対策](improvement_list/2026-09-21_llm_refusal_countermeasures.md)）。

`status='error'` は17件（6月〜9月）残っている。`--retry-errors` で再評価できるが、未評価のまま HRMOS で
人が判断済みの人も含むので、流す前に対象を確認すること。
...
### 中: 開発機の `config.yaml` に認証情報が平文で書かれている

HRMOS のパスワードと Resend の APIキーが `config.yaml` に直接書かれている（2026-09-21 確認）。
CLAUDE.md の規約（認証情報は環境変数 `HRMOS_PASSWORD` / `RESEND_API_KEY` に置く）に反する。
`.gitignore` 対象で、配布ビルドには機密スキャンがあるため流出経路は限られるが、フォルダごとの
コピーでは持ち出される。環境変数へ移して `config.yaml` から消すこと。
...
**2026-09-21 追記**: 住所のハイフン書き（`2-14-23` / `2丁目14-23`）は対応済み（実データで残り 399行→1行）。
1文字の名は引き続き対象外。以下はそれ以外の取りこぼし。
...
### 低: 評価失敗時のログに個人情報が残りうる（ログの保存期間が未定）

2026-09-21 から、AI の応答を評価結果として読めなかったときは応答全文を `data/logs/` に出している
（評価用の Claude CLI が会話を保存しなくなったため、拒否理由を読める唯一の記録）。AI が拒否文の中で
書類の住所などを引用すると、それもログに残る。`data/logs/` には削除の決まりが無い。
セキュリティレビューの MEDIUM 指摘。根拠: [改修履歴](improvement_list/2026-09-21_llm_refusal_countermeasures.md)


### 低: メールアドレスのマスクが、長い英数字の連続で極端に遅くなる

`pii_masker._EMAIL_PATTERN`（`[\w.+-]+@...`）は、`@` の無い英数字・カナの連続が長いと処理時間が
文字数の2乗で伸びる（2万文字の連続で 3〜13 秒。2026-09-21 測定、変更前のコードでも同じ）。
普通の書類では起きないが、細工した書類を1通送られると、その応募者のマスク処理で定期実行が
長時間止まりうる。直し方の候補: `@` を含む語だけを対象にする、または `\w` の連続に上限を付ける。
...
（`requirements.txt` には入れていない。配布先では不要なため）。2026-09-21 に `test_claude_client.py`・`test_notify.py` と
住所マスクのテストを足して91件。
...
- [2026-09-22 旧配布zipを管理対象から外し、例示の氏名を架空名に統一](improvement_list/2026-09-22_remove_old_zip_and_example_names.md)

- [2026-09-21 AI評価の拒否への対策](improvement_list/2026-09-21_llm_refusal_countermeasures.md) — 住所のハイフン書き番地のマスク（番地のあとは建物名・部屋番号までで止め、同じ行の職歴は残す）、評価AIをプロジェクト外・会話保存なし・道具なしで起動、評価できなかった応募者を結果メールに表示。`--safe-mode` を採用しなかった理由もここ。再評価で ○ になった自動NG登録済みの1人は、人が確認して NG のままとした
