...
HRMOS採用ページから応募者の書類（PDF/Word/Excel）を自動取得し、Claude AI / Gemini AIで評価基準に基づくスコアリング・面接質問生成を行い、結果をExcel一覧表にまとめるCLIツール。

### プログラム概要

本ツールは採用業務における書類選考の効率化を目的とする。以下の一連の処理を `python run.py scan` の1コマンドで自動実行する。

1. **HRMOS採用ページへの自動ログイン**
   - Playwright（Chromium）によるブラウザ自動化
   - 2段階ログイン（メールアドレス → パスワード）に対応
   - セッション情報を `storage_state.json` に保存し、次回以降の再ログインを省略

2. **応募者一覧の収集と書類ダウンロード**
   - 応募者一覧ページから全応募者のリンク・名前・IDを自動収集
   - 「さらに表示」ボタンの自動クリックで全件読み込み
   - 各応募者の個別ページから添付書類（PDF/Word/Excel）を自動ダウンロード

3. **書類のテキスト抽出**
   - PDF → pdfplumber、Word → python-docx、Excel → openpyxl でテキスト化
   - 1応募者に複数書類がある場合は全テキストを結合

4. **LLM（Claude / Gemini）による自動評価**
   - 結合テキスト + `config.yaml` の評価基準からプロンプトを構築
   - Claude CLI（`claude -p`）またはGemini CLIをsubprocessで呼び出し、JSON形式で評価結果を取得
   - `config.yaml` の `evaluation.provider` で切替可能（デフォルト: claude）
   - 各評価基準について1〜5点のスコアリングと評価コメントを生成
   - 応募者の性別・年齢を書類から読み取り
   - 面接質問候補を3つ自動生成（質問の観点は `config.yaml` で設定可）

5. **評価結果の保存とExcel出力**
   - SQLiteデータベースに全評価結果を蓄積（差分処理に対応）
   - Excel一覧表を自動生成（1行=1応募者のマトリクス形式、S/A/B/C/Dランク色付き）

6. **メール通知（オプション）**
   - Resend APIにより、評価完了時にサマリメールとExcel添付を自動送信

### 主な特徴

- **差分処理**: 未評価の応募者のみ自動処理。前回評価済みの応募者はスキップ
- **評価基準の自由定義**: `config.yaml` で評価項目・説明を自由に追加・変更可能
- **一貫した評価**: プロンプトにスコアリングルーブリックを明示し、応募者間のばらつきを抑制
- **タスクスケジューラ対応**: Windowsタスクスケジューラで定期自動実行が可能
- **セッション管理**: ブラウザセッションを保存し、毎回のログインを不要に
...
| AI評価 | Claude CLI / Gemini CLI via subprocess（configで切替）※後述 |
...
本ツールは `config.yaml` の `evaluation.provider` 設定に基づいて **Claude CLI** または **Gemini CLI** をsubprocessで呼び出してAI評価を行う。デフォルトはClaude CLI。

#### Claude CLI（デフォルト）
**Claude CLI（Claude Code）** の `claude -p`（パイプモード）を使用。APIキーは不要で、マシン上のCLIがログイン済みの **claude.ai アカウント**（Pro / Max プラン等）の認証情報をそのまま利用する。
...
#### Gemini CLI（オプション）
**Gemini CLI** をstdinパイプモードで使用。無料枠で利用可能。

```
Python subprocess
  └─▶ gemini (stdin にプロンプト送信)
        └─▶ Gemini API (Google アカウントで推論)
              └─▶ stdout にテキスト応答を返却
```

- **前提条件**: `npm install -g @google/gemini-cli` でインストール済みであること

#### 共通
- **応答形式**: プレーンテキスト（JSONを指示しているが保証はないため、パーサーで吸収）
- **切替方法**: `config.yaml` の `evaluation.provider` を `"claude"` または `"gemini"` に設定
...
│   │   ├── gemini_client.py        #   Gemini CLI subprocess呼び出し
...
└─────────────────────────┬───────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────┐
│               llm_client.py (プロバイダー切替)             │
│                                                          │
│   provider="claude" → claude_client.py                   │
│     subprocess.run(["claude", "-p"], input=prompt)       │
│   provider="gemini" → gemini_client.py                   │
│     subprocess.run(["gemini"], input=prompt)             │
│                                                          │
...
evaluation:               # AI評価の実行パラメータ
  provider: "claude"      #   LLMプロバイダー ("claude" or "gemini")
  system_instructions:    #   プロンプトに含むシステム指示
  max_retries: 3          #   LLM CLIリトライ回数
...
| evaluator/llm_client | タイムアウト/CLIエラー | 最大3回リトライ後、該当応募者をスキップ |
